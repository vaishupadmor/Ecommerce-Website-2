...
function Cart() {
    ...
    return (
      <div className='fixed top-0 left-0 w-full h-full bg-black bg-opacity-50 flex justify-center items-center' onClick={onClose}>
        <div className='bg-slate-200 p-10 rounded-lg w-[400px]' onClick={(e) => { e.stopPropagation() }}>
          <h1>Checkout</h1>
          <Input label="Name" placeholder='Enter your name' val={name} onChange={(val) => setName(val)} />
          <Input label="Address" placeholder='Enter your address' val={address} onChange={(val) => setAddress(val)} />
          <Input label="Phone" placeholder='Enter your Phone Number' val={phone} onChange={(val) => setPhone(val)} />
          ...
          <div className='mt-10 flex justify-center'>
            <Button label="Complete Order" onClick={() => {
  ...
  const PaymentDialog = ({ isPaymentOpen, onClose }) => {
    ...
    return (
      <div className='fixed top-0 left-0 w-full h-full bg-white flex justify-center items-center z-60'>
          ...
          {cart.map((product) => {
            ...
            return (
              <div className='bg-white shadow-lg rounded-lg overflow-hidden m-5 px-10 py-5 w-full md:w-2/3 relative flex' key={productId}>
                ...
                <button className='absolute top-5 right-5 bg-red-500 text-sm px-5 py-1 text-white rounded-lg' onClick={() => removeItemFromCart(productId)}>Remove</button>
              </div>
            );
          })}
        </div>
